...
while True:
    ...
    while True:
        #sample
        dna_storage.sample_reads_indel(tmpfile+'.oligos',tmpfile+'.reads',num_reads,sub_prob=sub_prob, del_prob=del_prob, ins_prob=ins_prob, frac_random_reads=frac_random_reads)
        #decode
        dna_storage.decode_data(tmpfile+'.reads',oligo_length,tmpfile+'.decoded',bin_index_len,BCH_bits,LDPC_alpha,ldpc_code_prefix,file_size,0.04,sync=sync,sync_pos=sync_pos)
        num_it += 1
...
os.remove(tmpfile+'.oligos')
os.remove(tmpfile+'.reads')
os.remove(tmpfile+'.decoded')
