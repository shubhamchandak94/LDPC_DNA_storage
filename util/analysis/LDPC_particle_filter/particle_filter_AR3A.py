...
def main():
	...
	alpha = 1.0
	eps = 0.00001
	for cov in [1.6]:#[1.6,1.62,1.64,1.66,1.68,1.7]:
		print(cov)
		lamb = cov/(1+alpha)
...
def sampled_DE(lamb, num_iter, N, eps):
	...
	for t in range(num_iter):
		...
		sum_a = np.sum(P_hat_a_2[nbr_a],axis=1)
		sum_b_1 = P_hat_b_2[nbr_b[:,0]]
		sum_b = np.sum(P_hat_b_2[nbr_b],axis=1)
		sum_c = np.sum(P_hat_c_2[nbr_c],axis=1)	
		P_2_a = l_Y_2 + sum_b + sum_c
		P_2_b = l_Y_2 + sum_b_1 + sum_a + sum_c 
		P_2_c = l_Y_2 + sum_a + sum_b
		...
		prod_2 = np.prod(np.tanh(P_2_a[nbr_2]/2),axis=1)
		prod_5_1 = np.tanh(P_5_a[nbr_5[:,0]]/2)
		prod_5 = np.prod(np.tanh(P_5_a[nbr_5]/2),axis=1)
		
		P_hat_a_1 = 2*np.arctanh(prod_2*prod_5)
		P_hat_a_2 = 2*np.arctanh(prod_1*prod_5)
		P_hat_a_5 = 2*np.arctanh(prod_1*prod_2*prod_5_1)
		...
		P_hat_b_3 = 2*np.arctanh(prod_2*prod_4*prod_5)
		...
		prod_5_1 = np.tanh(P_5_c[nbr_5[:,0]]/2)
		...
		P_hat_c_5 = 2*np.arctanh(prod_2*prod_3*prod_4*prod_5_1)
